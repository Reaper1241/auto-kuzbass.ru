...
const fetchCars = async (page = 1) => {
  ...
  url.searchParams.set('car_tag_id', '15');
...
const title = computed(() => {
  if ($route.params.car) {
    ...
    return `Внедорожники автомобили ${cleanUpTitle($route.params.brand, $route.params.model).toUpperCase()} в наличии`;
  } else if ($route.params.brand) {
    return `Внедорожники автомобили ${cleanUpTitle($route.params.brand).toUpperCase()} в наличии`;
  } else {
    return 'Внедорожники в наличии';
  }
});
