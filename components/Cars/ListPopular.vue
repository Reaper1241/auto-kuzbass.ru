...
const fetchCars = async (page = 1) => {
  ...
  url.searchParams.set('car_tag_id', '12');
...
const title = computed(() => {
  if ($route.params.car) {
    ...
    return `Популярные автомобили ${cleanUpTitle($route.params.brand, $route.params.model).toUpperCase()} в наличии`;
  } else if ($route.params.brand) {
    return `Популярные автомобили ${cleanUpTitle($route.params.brand).toUpperCase()} в наличии`;
  } else {
    return 'Популярные автомобили в наличии';
  }
});
  ...
  <section class="section" v-else-if="!isLoading && carsCount === 0">
    <div class="container">
      <div class="no-cars-message">
        <h3>Популярные автомобили не найдены</h3>
