...
const fetchCars = async (page = 1) => {
  ...
  url.searchParams.set('car_tag_id', '16');
...
const title = computed(() => {
  if ($route.params.car) {
    ...
    return `Седаны ${cleanUpTitle($route.params.brand, $route.params.model).toUpperCase()} в наличии`;
  } else if ($route.params.brand) {
    return `Седаны ${cleanUpTitle($route.params.brand).toUpperCase()} в наличии`;
  } else {
    return 'Седаны в наличии';
  }
});
