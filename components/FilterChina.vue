...
// ОТЛАДКА: смотрим что приходит в chinaBrands
console.log('FilterChina - appStore.chinaBrands:', appStore.chinaBrands);

// ИСПРАВЛЕНО: правильное преобразование брендов для BaseSelect
const brands = computed(() => {
    console.log('Computing brands from:', appStore.chinaBrands);
    
    let brandsData = [];
    
    // Проверяем разные возможные форматы данных
    if (Array.isArray(appStore.chinaBrands)) {
        brandsData = appStore.chinaBrands;
    } else if (appStore.chinaBrands?.brands) {
        brandsData = appStore.chinaBrands.brands;
    } else if (appStore.chinaBrands?.data) {
        brandsData = appStore.chinaBrands.data;
    } else {
        console.log('No brands data found');
        return [];
    }
    
    console.log('Raw brands data:', brandsData);
    
    // Преобразуем в формат для BaseSelect: { id, name }
    const formattedBrands = brandsData.map(brand => {
        // Пробуем разные возможные поля для названия
        const name = brand.brand || brand.name || brand.title || 'Неизвестно';
        console.log(`Brand ${brand.id}:`, brand, '-> name:', name);
    ...
    console.log('Formatted brands for select:', formattedBrands);
...
// Вычисляемое свойство для проверки, выбраны ли марка и модель
const isBrandAndModelSelected = computed(() => {
    return brand.value > 0 && model.value > 0;
});

// Проверяем загружены ли китайские бренды
const isBrandsLoaded = computed(() => {
...
function getFilterData(recentStatus) {
    if (!isBrandsLoaded.value) {
        console.log('China brands not loaded yet, waiting...');
        return;
    }
    ...
    // Базовый URL с фильтром по тегу id=3 для китайских авто
    let baseUrl = `${apiNew}filters/params?car_tag_id=3`;
    
    // Добавляем параметры бренда и модели если они есть в роуте
    if ($route.params.brand) {
        const brandId = getBrand($route.params.brand);
        if (brandId) baseUrl += `&brand_id=${brandId}`;
    }
    if ($route.params.model) {
        const modelId = getModel($route.params.brand, $route.params.model);
        if (modelId) baseUrl += `&car_model_id=${modelId}`;
    }
...
function updateQuery() {
    // Всегда добавляем car_tag_id=3 в запрос для китайских авто
    fullQuary = [
        `car_tag_id=3`,
        ...
        `${body.value > 0 ? `body_type_id=${body.value}` : ''}`,
...
// Ищем бренд только среди китайских
function getBrand(brandName) {
    ...
    return brandsData.find(brand => brand.url_brand === brandName)?.id;
}

// Ищем модель только среди китайских брендов
function getModel(brandName, modelName) {
    ...
    const foundBrand = brandsData.find(brand => brand.url_brand === brandName);
    return foundBrand?.car_models?.find(model => model.url_model === modelName)?.id;
}
...
async function reset() {
    ...
    fullQuary = `car_tag_id=3`;

    // Перезагружаем фильтр
    await getFilterData(false);
...
function updateFilter(from) {
    ...
    const queryUrl = fullQuary.includes('car_tag_id=3') ? fullQuary : `car_tag_id=3&${fullQuary}`;
    
    fetchClientWrapper(`${apiNew}filters/params?${queryUrl}`)
        .then(res => res.json())
        .then(data => quary.value = data)
...
// Ждем загрузки китайских брендов перед инициализацией фильтра
watch(() => appStore.chinaBrands, (newVal) => {
    console.log('China brands updated in watcher:', newVal);
    if (newVal && (Array.isArray(newVal) || newVal.brands || newVal.data)) {
        getFilterData(true);
    }
}, { immediate: true });

// Инициализация фильтра когда бренды уже загружены
onMounted(() => {
    console.log('FilterChina mounted, china brands:', appStore.chinaBrands);
    if (appStore.chinaBrands && (Array.isArray(appStore.chinaBrands) || appStore.chinaBrands.brands || appStore.chinaBrands.data)) {
        getFilterData(true);
    }
});
</script>
